    ...
    log.atInfo()
        .setMessage(
            "Register certificate '%s' received"
                .formatted(registerCertificateDTO.getIntyg().getIntygsId().getExtension()))
        .addKeyValue("event.logical_address", logicalAddress)
        .addKeyValue(
            "event.certificate.id", registerCertificateDTO.getIntyg().getIntygsId().getExtension())
        .addKeyValue(
            "event.answered.message.id",
            registerCertificateDTO.getSvarPa() != null
